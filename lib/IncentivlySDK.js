...
class IncentivlySDK {
    constructor() {
        ...
        this.apiClient = new APIClient_1.APIClient(undefined, this.loggingEnabled);
    }
    /**
     * Initialize the SDK
     ...
     */
    async updateUserIdentifier(newUserIdentifier) {
        ...
        if (!currentUserIdentifier) {
            throw new Errors_1.UserNotRegisteredError();
        }
        if (!devKey) {
            throw new Errors_1.DevKeyNotFoundError();
        }
        try {
            const response = await this.apiClient.updateUserIdentifier(currentUserIdentifier, newUserIdentifier, devKey);
            if (response.success) {
     ...
     */
    async reportPaymentIOS(productId, iosTransactionId) {
        ...
        if (isProcessed) {
            ...
            throw new Errors_1.TransactionAlreadyProcessedError();
        }
        const userIdentifier = await this.storageManager.getUserIdentifier();
        const devKey = await this.storageManager.getDevKey();
        if (!userIdentifier) {
        ...
        try {
            ...
            const response = await this.apiClient.reportPaymentIOS(userIdentifier, productId, iosTransactionId, devKey);
            if (response.success) {
            ...
            if (error instanceof Errors_1.TransactionAlreadyProcessedError) {
     ...
     */
    async reportPaymentAndroid(productId, androidPurchaseToken, androidPackageName, androidOrderId) {
        ...
        try {
            ...
            const response = await this.apiClient.reportPaymentAndroid(userIdentifier, productId, androidPurchaseToken, androidPackageName, androidOrderId, devKey);
            if (response.success) {
                // Mark as processed locally
...
exports.IncentivlySDK = IncentivlySDK;
IncentivlySDK.instance = null;
